...
test("remain on the same location", () => {
  ...
  expect(bookingsResult[1].facilitator).toMatchObject(facilitators[1]);
  expect(bookingsResult[1].guestSpeaker).toMatchObject(guestSpeakers[1]);

  expect(bookingsResult[2].facilitator).toMatchObject(facilitators[0]);
  expect(bookingsResult[2].guestSpeaker).toMatchObject(guestSpeakers[0]);
  ...
  if (bookingsResult[0].facilitator instanceof UserModel) {
  ...
  } else if (bookingsResult[1].facilitator instanceof UserModel) {
    const user = bookingsResult[1].facilitator as User;
    if (user._facilitator instanceof FacilitatorModel) {
      const facilitator = user._facilitator as Facilitator;
      expect(facilitator.assignedTimes[0].availableFrom).toMatchObject(bookingsResult[1].sessionTime.timeBegin);
      expect(facilitator.assignedTimes[0].availableUntil).toMatchObject(bookingsResult[1].sessionTime.timeEnd);
    }
  } else if (bookingsResult[2].facilitator instanceof UserModel) {
    const user = bookingsResult[2].facilitator as User;
    if (user._facilitator instanceof FacilitatorModel) {
      const facilitator = user._facilitator as Facilitator;
      expect(facilitator.assignedTimes[1].availableFrom).toMatchObject(bookingsResult[2].sessionTime.timeBegin);
      expect(facilitator.assignedTimes[1].availableUntil).toMatchObject(bookingsResult[2].sessionTime.timeEnd);
    }
    ...
    if (user._guestSpeaker instanceof GuestSpeakerModel) {
      const guestSpeaker = user._guestSpeaker as GuestSpeaker;
      expect(guestSpeaker.assignedTimes[0].availableFrom).toMatchObject(bookingsResult[1].sessionTime.timeBegin);
      expect(guestSpeaker.assignedTimes[0].availableUntil).toMatchObject(bookingsResult[1].sessionTime.timeEnd);
    }
  } else if (bookingsResult[2].guestSpeaker instanceof UserModel) {
    const user = bookingsResult[2].guestSpeaker as User;
    if (user._guestSpeaker instanceof GuestSpeakerModel) {
      const guestSpeaker = user._guestSpeaker as GuestSpeaker;
      expect(guestSpeaker.assignedTimes[1].availableFrom).toMatchObject(bookingsResult[2].sessionTime.timeBegin);
      expect(guestSpeaker.assignedTimes[1].availableUntil).toMatchObject(bookingsResult[2].sessionTime.timeEnd);
    }
  }
});
